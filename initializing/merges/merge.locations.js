var fs = require("fs"),
	...
	merging = require("./source/locations.json").export,
	exporting = [],
	modMap = {},
	locked = {},
	addObjects,
...
for(i=0; i<merging.length; i++) {
	...
	try {
		...
		if(merged.container) {
			if(merged.container && merged.container.indexOf("continent") === -1 && merged.container !== "undefined") {
				merged.location = merged.container;
			} else {
...
fs.writeFile("locations.json", JSON.stringify({"import": exporting}, null, "\t"), () => {});
