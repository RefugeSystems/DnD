var feetToStuds = /([0-9]+)([ -])(feet|foot)/g,
	nonalpha = /[^a-z0-9]/ig,
	exporting = [],
	names,
	spells = {
...
names = Object.keys(spells);
names.forEach(function (name) {
	var spell = spells[name];
	spell.name = name;
	spell.id = "spell:" + name.toLowerCase().replace(nonalpha, "");
    if (spell.range === "Touch") {
        spell.range_normal = 0;
    } else {
        spell.range_normal = parseInt(spell.range) * .8;
    }
    spell.cast_time = parseTime(spell.casting_time);
	if(spell.school) {
		spell.type = "type:magic:" + spell.school.toLowerCase();
	} else {
		spell.type = "type:magic";
	}
	spell.review = true;
    spell.cast_uses_somatic = spell.components ? spell.components.indexOf("S,") !== -1 || spell.components.indexOf(",S") !== -1 : false;
    spell.cast_uses_items = spell.components && spell.components.indexOf("M") !== -1 ? ["item:material:spell:free"] : null;
    spell.cast_uses_verbal = spell.components ? spell.components.indexOf("V,") !== -1 || spell.components.indexOf(",V") !== -1 : false;
    spell.action_cost = {};
    if (spell.casting_time && spell.casting_time.indexOf("action") !== -1) {
        spell.action_cost["main"] = 1;
    } else if (spell.casting_time && spell.casting_time.indexOf("bonus") !== -1) {
        spell.action_cost["bonus"] = 1;
    } else if (spell.casting_time && spell.casting_time.indexOf("reaction") !== -1) {
        spell.action_cost["reaction"] = 1;
    }
    if (spell.duration) {
		spell.duration = spell.duration.toLowerCase();
		if(spell.duration.indexOf("concentration") !== -1) {
			spell.concentration = true;
			spell.duration = spell.duration.replace("concentration, ", "");
		}
		spell.duration = spell.duration.replace(/[, ]*up to[, ]*/, "");
		spell.duration = parseTime(spell.duration);
    }
	if(spell.level > 7) {
		exporting.push(spell);
	}
	
});

function cleanDesription(description) {
    var result = description,
    match;
    while (match = feetToStuds.exec(description)) {
        result = result.replace(match[0], parseInt(match[1]) * .8 + match[2] + "stud" + (match[2] === "-" ? "" : "s"));
    }
    return result;
}

function parseTime(time) {
	if(time === "Instantaneous" || time === "instantaneous") {
		return 0;
	}
	if(!time) {
		return null;
