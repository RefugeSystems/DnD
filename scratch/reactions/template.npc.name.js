...
universe.nameNPCs = function(template) {
	...
	*/
	...
	utility.log("T1: " + (!template.name || template.name.length < 6 || template.name.indexOf("NPC") !== -1 || template.name.indexOf("npc") !== -1));
	utility.log("T2: " + mapping[template.race]);
	utility.log("T3: " + mapping[template.race][template.gender]);
	utility.log("T4: " + universe.get(mapping[template.race][template.gender]));
	utility.log("T5: " + utility.getRandomName(dataset));
